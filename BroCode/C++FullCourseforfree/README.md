...
Welcome to my personal learning repository for mastering C++! This repository contains all the code, practice exercises, notes, and projects written while following the **[C++ Full Course for Beginners by Bro Code](https://www.youtube.com/watch?v=-TkoO8Z07hI)** on YouTube.
...
To run these C++ programs on Windows, follow these instructions using `g++` (MinGW) or any compatible C++ compiler:

### 1. Compilation
Navigate to the directory containing the source code and compile using:
```bash
g++ C++Programs/helloworld.cpp -o C++Programs/helloworld.exe
```
...
Here is a checklist mapping all the topics from the course to track what has been covered so far:

### Phase 1: The Basics (Fundamentals)
- [x] **01. C++ Tutorial for Beginners** 🏁 — *Setup, compilation, basic output with `std::cout`*
- [x] **02. Variables & Basic Data Types** 🗃️ — *Integers, doubles, characters, booleans, strings*
...
### 1. Namespaces
Avoids name collisions in large scale C++ projects.
```cpp
namespace first {
    int x = 1;
}
...
C++11 alias syntax is preferred over standard C `typedef` as it works better with templates.
```cpp
